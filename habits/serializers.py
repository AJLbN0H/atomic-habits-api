...
class HabitsSerializer(ModelSerializer):
    ...
    def validate(self, data):
        ...
        - chat id должен состоять из 10 цифр"""
        ...
        if associated_habit:
            if not associated_habit.a_sign_of_a_pleasant_habit:
                raise serializers.ValidationError(
                    "Привычка, которую вы выбрали не является приятной"
                )

        if a_sign_of_a_pleasant_habit:
            if associated_habit or reward:
                raise serializers.ValidationError(
                    "Для приятной привычки нельзя выбрать вознаграждение или еще одну приятную привычку"
                )
        ...
        if chat_id:
            if chat_id.isdigit():
                if len(chat_id) != 10:
                    raise serializers.ValidationError(
                        "chat_id должен состоять из 10 цифр"
                    )
            else:
                raise serializers.ValidationError("chat_id должен состоять из 10 цифр")
